...
def _omni(**chat_kwargs) -> OmniRequest:
    ...
    return OmniRequest(
        inputs=_build_chat_generate_request(
            ChatCompletionRequest(model="m", **chat_kwargs)
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--fixtures-dir", required=True, help="tests/fixtures/moss_speech"
    )
    ...
    out["voc"] = {
        ...
        "hash_vs_p1_reference": blake2b(torch.tensor(vs.audio_samples))
    ...
    out["voc"]["same_seed_diff_request_equal"] = blake2b(
        torch.tensor(vs2.audio_samples)
    ) == blake2b(torch.tensor(vs.audio_samples))
    ...
    out["voc"]["diff_seed_differs"] = blake2b(
        torch.tensor(vs3.audio_samples)
    ) != blake2b(torch.tensor(vs.audio_samples))
    ...
    out["text_decode"] = {
        "matches_fixture": tds.generated_text == expected,
        "text_head": tds.generated_text[:40],
    }

    # CPU preflight remains a separate component regression after P3.
    parsed = validate_ar_preconditions(args.model_path, dtype="bfloat16")
    out["ar_preflight"] = {"config_type": type(parsed).__name__}
    ...
    assert (
        out["voc"]["rng_restored_py"]
        and out["voc"]["rng_restored_np"]
        and out["voc"]["rng_restored_torch"]
    )
    assert (
        out["voc"]["same_seed_diff_request_equal"] and out["voc"]["diff_seed_differs"]
    )
    assert out["text_decode"]["matches_fixture"]
    assert out["ar_preflight"]["config_type"] == "MossSpeechConfig"
