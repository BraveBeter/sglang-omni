...
def main() -> None:
    ...
    with forward_context(ForwardContext(attn_backend=runner.attn_backend)):
        ...
        out = model(input_ids=fb.input_ids, positions=fb.positions, forward_batch=fb,
    ...
    with torch.no_grad():
        ...
        rep["embed_discriminator"] = {
            "forward_rows_norm": [float(via_forward[i].float().norm()) for i in range(2)],
    ...
    rep["top20_overlap"] = {"text": topk_overlap(nt, ref_t), "audio": topk_overlap(na, ref_a)}
    rep["max_abs"] = {"text": float((nt - ref_t).abs().max()), "audio": float((na - ref_a).abs().max())}
    ...
    with forward_context(ForwardContext(attn_backend=runner.attn_backend)):
        runner.attn_backend.init_forward_metadata(forward_batch=fb)
        out2 = model(input_ids=fb.input_ids, positions=fb.positions, forward_batch=fb,
                     input_embeds=embeds.to(runner.device))
    for h in hs:
        h.remove()
    ...
    def cmp(name, native_t, ref_t):
        ...
        return {"cos_last": float(cos), "max_abs": float((n - r).abs().max()),
                "n_norm": float(n.norm()), "r_norm": float(r.norm())}
    ...
    wrep = {}
    for ck, attr in (("model.embed_tokens.weight", "embed_tokens"),
        ...
        wrep[attr] = {
            "ckpt_shape": list(ck_w.shape), "native_shape": list(native_w.shape),
            "rows_equal": bool(torch.equal(native_w[:3].to(ck_w.dtype), ck_w[:3].to(native_w.dtype) if False else ck_w[:3].to(native_w.dtype))),
            "first_row_max_abs": float((native_w[0] - ck_w[0].float()).abs().max()),
            "native_norm": float(native_w.norm()), "ckpt_norm": float(ck_w.float().norm()),
        }
    rep["weights"] = wrep
                ...
                if isinstance(out, tuple) and len(out) == 2 and torch.is_tensor(out[0]):
                    ...
                    if name == "shared_0":
                        ...
                        caps2["r0_dump"] = {
                            ...
                            "r_dtype": str(out[1].dtype), "r_shape": list(out[1].shape),
                        }
        return hook

    caps2: dict = {}
    hp = [model.layers[0].register_forward_hook(snap_pair("shared_0")),
    ...
    with forward_context(ForwardContext(attn_backend=runner.attn_backend)):
        runner.attn_backend.init_forward_metadata(forward_batch=fb)
        model(input_ids=fb.input_ids, positions=fb.positions, forward_batch=fb,
              input_embeds=embeds.to(runner.device))
    for h in hp:
        h.remove()

    bisect2 = {}
    for k, rv in (("shared_0", ref_blob["shared_0"][0]), ("shared_1", ref_blob["shared_1"][0]),
                  ("shared_31", ref_blob["shared_31"][0]), ("text_last", ref_blob["text_last"][0]),
        ...
        bisect2[k] = {"cos_last": float(cos), "max_abs": float((n[L - 1] - rv[L - 1].float()).abs().max()),
                      "n_row_norm": float(n[L - 1].norm()), "r_row_norm": float(rv[L - 1].float().norm())}
    rep["layer_bisect"] = bisect2
    rep["layer_hr_norms"] = {k: v for k, v in caps2.items() if k.endswith("__h") or k.endswith("__r")}
    ...
    with torch.no_grad():
        ...
        audit["o_match"] = bool(torch.equal(nv["self_attn.o_proj.weight"].detach().float().cpu(), ckT["o"]))
        audit["qnorm_match"] = bool(torch.equal(nv["self_attn.q_norm.weight"].detach().float().cpu(), ckT["qn"]))
        audit["ln1_match"] = bool(torch.equal(nv["input_layernorm.weight"].detach().float().cpu(), ckT["ln1"]))
        gu = nv["mlp.gate_up_proj.weight"].detach().float().cpu()
        audit["gate_match"] = bool(torch.equal(gu[:12288], ckT["gate"]))
        audit["down_match"] = bool(torch.equal(nv["mlp.down_proj.weight"].detach().float().cpu(), ckT["down"]))
    ...
    hrope = model.layers[0].self_attn.rotary_emb.register_forward_hook(snapnorm("rope_out"))
    with forward_context(ForwardContext(attn_backend=runner.attn_backend)):
        runner.attn_backend.init_forward_metadata(forward_batch=fb)
        model(input_ids=fb.input_ids, positions=fb.positions, forward_batch=fb,
              input_embeds=embeds.to(runner.device))
