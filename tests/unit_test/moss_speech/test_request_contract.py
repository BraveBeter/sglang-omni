...
def test_valid_audio_request_with_inline_part() -> None:
    omni = _lower(
        modalities=["audio"],
        messages=[{"role": "user", "content": [_audio_part(_b64())]}],
    )
...
def test_audios_list_binds_one_to_one_in_order(tmp_path) -> None:
    ...
    gen2 = _build_chat_generate_request(
        ChatCompletionRequest(
            model="moss-speech",
            messages=[
...
def test_mixed_modality_in_single_turn_rejected() -> None:
    omni = _lower(
        messages=[
            {
                "role": "user",
                "content": [{"type": "text", "text": "hi"}, _audio_part(_b64(0.1))],
            }
        ]
    )
...
def test_two_audio_parts_in_one_message_rejected() -> None:
    omni = _lower(
        messages=[
            {
                "role": "user",
                "content": [_audio_part(_b64(0.1)), _audio_part(_b64(0.1))],
            }
        ]
    )
...
def test_unknown_role_rejected() -> None:
    gen = _build_chat_generate_request(
        ChatCompletionRequest(model="m", messages=[{"role": "tool", "content": "x"}])
    )
...
def test_undecodable_audio_rejected() -> None:
    omni = _lower(
        messages=[
            {
                "role": "user",
                "content": [_audio_part(base64.b64encode(b"not-audio").decode())],
            }
        ]
    )
...
def test_oversize_duration_rejected() -> None:
    omni = _lower(
        messages=[{"role": "user", "content": [_audio_part(_b64(seconds=31.0))]}]
    )
...
def test_url_audio_rejected() -> None:
    omni = _lower(
        messages=[
            {"role": "user", "content": [_audio_part("https://example.com/x.wav")]}
        ]
    )
...
def test_route_fn_selects_exactly_one_terminal() -> None:
    ...
    assert (
        rb.resolve_output_terminal("r1", types.SimpleNamespace(data=st))
        == "text_decode"
    )
    st2 = MossSpeechState(output_modality="audio")
    assert (
        rb.resolve_output_terminal("r2", types.SimpleNamespace(data=st2))
        == "audio_vocoder"
    )
    assert (
        rb.resolve_output_terminal(
            "r3", types.SimpleNamespace(data={"output_modality": "audio"})
        )
        == "audio_vocoder"
    )
    with pytest.raises(rb.RequestValidationError):
        rb.resolve_output_terminal(
            "r4", types.SimpleNamespace(data=MossSpeechState(output_modality="both"))
        )
...
def test_unsupported_or_nonfinite_sampling_rejected_before_codec(params):
    with pytest.raises(rb.RequestValidationError):
        _normalize(_lower(**params))
