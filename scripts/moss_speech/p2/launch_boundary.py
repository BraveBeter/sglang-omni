#!/usr/bin/env python3
"""Load-precondition check for MOSS-Speech (T2.5).

Runs the SAME configuration-loading, registry and DAG-validation path as the
CLI (``ConfigManager.from_file``), then:

  A. positive: YAML parse -> merged PipelineConfig -> registry resolution by
     model_path -> hf_config parse of the locked checkpoint -> real server
     args via the formal ar factory's precondition block -> tagged
     MossSpeechARNotImplemented (expected; P3 pointer). "Load-precondition
     PASSED" means everything before model loading is verified — NOT that
     serving works.
  B. negative: invalid config field, missing model dir, bad stage edge,
     terminal-with-route_fn — each must fail with the framework's own error.

Run on the login node (CPU-only path).
"""
...
def main() -> None:
    ...
    parser.add_argument("--config", required=True, help="examples/configs/moss_speech.yaml")
    ...
    try:
        ...
        assert all(next(s for s in config.stages if s.name == n).terminal for n in ("text_decode", "audio_vocoder"))
    ...
    try:
        ...
        _ok("hf_config_parse", {"type": type(parsed).__name__, "layers": parsed.num_hidden_layers})
    except Exception as exc:  # noqa: BLE001
        _fail("hf_config_parse", {"error": f"{type(exc).__name__}: {exc}"})

    # ---------------- A5: formal AR factory boundary ---------------------------
    try:
        ...
        pre_args = {"dtype": "bfloat16", "codec_path": None}
        for stage in config.stages:
            if stage.name == "ar_engine":
                pre_args.update(stage.factory_args or {})
        try:
            create_ar_engine_executor(config.model_path, **pre_args)
            _fail("ar_boundary", {"error": "factory returned without the P3 boundary error"})
        except MossSpeechARNotImplemented as exc:
            _ok("ar_boundary", {"raised": True, "message_head": str(exc)[:140]})
        except Exception as exc:  # noqa: BLE001
            _fail("ar_boundary", {"error": f"unexpected failure type {type(exc).__name__}: {exc}"})
    except Exception as exc:  # noqa: BLE001
        _fail("ar_boundary", {"error": f"{type(exc).__name__}: {exc}"})
    ...
    try:
        ...
        from sglang_omni.models.moss_speech.stages import MossSpeechARNotImplemented  # noqa: F401

        # Framework-validated constructs (schema.py _validate_general):
        try:
            PipelineConfig(model_path="m", stages=[
                StageConfig(name="t", process="p", factory="builtins:print", terminal=True,
                            next="x", route_fn="builtins:print"),
            ])
        ...
        try:
            PipelineConfig(model_path="m", stages=[
                StageConfig(name="a", process="p", factory="builtins:print", next="ghost"),
            ])
            negative["next_missing_stage"] = "NO ERROR (construct-time unknown-target not validated; runtime routing would reject)"
        except Exception as exc:  # noqa: BLE001
        ...
        hard_failures = {k: v for k, v in negative.items() if v.startswith("NO ERROR") and k == "terminal_route_fn"}
        if hard_failures:
