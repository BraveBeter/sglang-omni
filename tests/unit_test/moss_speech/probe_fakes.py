...
def create_probe_preprocessing(model_path: str):
    def compute(payload: StagePayload) -> StagePayload:
        from sglang_omni.models.moss_speech.request_builders import normalize_and_validate
        ...
        print(f"[probe-pre] done {payload.request_id} modality={state.output_modality}", flush=True)
...
def create_probe_ar(model_path: str):
    def compute(payload: StagePayload) -> StagePayload:
        ...
        state = payload.data if isinstance(payload.data, MossSpeechState) else MossSpeechState.from_dict(payload.data)
        state.output_grid = [[151645, 0]] if state.output_modality == "text" else [[151667, 100]]
        return StagePayload(request_id=payload.request_id, request=payload.request, data=state.to_dict())
...
def create_probe_text_terminal(model_path: str):
    def compute(payload: StagePayload) -> StagePayload:
        ...
        state = payload.data if isinstance(payload.data, MossSpeechState) else MossSpeechState.from_dict(payload.data)
        print(f"[probe-terminal] got {payload.request_id} modality={state.output_modality}", flush=True)
