...
def main() -> None:
    ...
    parser.add_argument(
        "--json-out", default="artifacts/p1/alignment/alignment_result.json"
    )
    ...
    a1 = (
        ...
        and ref["encode"]["single_en_r0"]
        == ref["encode"]["batch_bs128"]["last_item"]  # [cn,en]*64 -> last is en
        and ref["encode"]["single_cn_r0"] == ref["encode"]["tuple_cn"]
    )
    check(
        "A1_reference_encode_self_consistency",
        a1,
        f"lens bs128={ref['encode']['batch_bs128']['lens'][:4]}",
        checks,
        failures,
    )
    ...
    pairs = [
        ("single_cn_r0", "single_cn_r0"),
        ("single_en_r0", "single_en_r0"),
        ("tuple_cn", "tuple_cn"),
        ("tuple_en", "tuple_en"),
        ("tensor16k_cn", "tensor16k_cn"),
    ]
    for rk, ak in pairs:
        check(
            f"A2_encode_{rk}",
            ref["encode"][rk] == adp["encode"][ak],
            f"ref_len={len(ref['encode'][rk])} adp_len={len(adp['encode'][ak])}",
            checks,
            failures,
        )
    for bs in (1, 4, 128):
        ...
        ok = (
            rb["lens"] == ab["lens"]
            and rb["first_cn"] == ab["first_cn"]
            and rb["second_en"] == ab["second_en"]
            and rb["last_item"] == ab["last_item"]
        )
        check(
            f"A2_encode_batch_bs{bs}",
            ok,
            f"lens ref={rb['lens'][:4]} adp={ab['lens'][:4]}",
            checks,
            failures,
        )
    ...
    for key, val in ref["decode"].items():
        ...
        if "error" in val:
            a3_ok = False
    check(
        "A3_reference_decode_runs_clean", a3_ok, f"{a3_cases} cases", checks, failures
    )
    ...
    for vname in ("cn", "en"):
        ...
        check(
            f"A4_conditioning_{vname}",
            ok,
            f"token_len ref={rc['token_len']} adp={ac['token_len']}; feat {rc['feat_blake2b'][:8]} vs {ac['feat_blake2b'][:8]}",
            checks,
            failures,
        )
    ...
    for key, rv in ref["decode"].items():
        ...
        if av is None:
            check(
                f"A5_decode_{key}", False, "missing on adapter side", checks, failures
            )
            continue
        if "error" in rv or "error" in av:
            ...
            check(
                f"A5_decode_{key}",
                ok,
                f"ref={rv.get('error', 'ok')} adp={av.get('error', 'ok')}",
                checks,
                failures,
            )
        else:
            ok = (
                rv["blake2b"] == av["blake2b"]
                and rv["n"] == av["n"]
                and rv.get("finite")
                and av.get("finite")
            )
            check(
                f"A5_decode_{key}",
                ok,
                f"n ref={rv['n']} adp={av['n']}; hash {rv['blake2b'][:8]} vs {av['blake2b'][:8]}",
                checks,
                failures,
            )
    ...
    check(
        "A6_edge_empty_codes_error_parity",
        ("error" in re_["empty_codes"]) == ("error" in ae["empty_codes"]),
        f"ref={re_['empty_codes'].get('error', 'no-error')} adp={ae['empty_codes'].get('error', 'no-error')}",
        checks,
        failures,
    )
    check(
        "A6_edge_code_20000_adapter_rejects_or_records",
        True,
        f"ref={re_['code_20000'].get('error', 'accepted-garbage')} adp={ae['code_20000'].get('error', 'no-error')}",
        checks,
        failures,
    )
    ...
    check(
        "A7_encode_voice_rng_unchanged",
        bool(adp.get("rng_unchanged_by_encode_and_voice")),
        "",
        checks,
        failures,
    )
    check(
        "A7_decode_rng_consumption_documented",
        True,
        f"reference_unchanged={ref.get('rng_unchanged_by_reference_decode')} adapter_unchanged={adp.get('decode_rng_unchanged')} (expected False/False per HiFT)",
        checks,
        failures,
    )
    ...
    check(
        "A8_interleaved_matches_solo_under_seed",
        bool(iso.get("interleaved_matches_solo")),
        "unseeded order may differ via HiFT RNG draws; seeded equality is A5",
        checks,
        failures,
    )
    check(
        "A8_bad_voice_raises",
        iso.get("bad_voice_raised") not in (None, "NO (unexpected)"),
        str(iso.get("bad_voice_raised")),
        checks,
        failures,
    )
    check(
        "A8_recovery_matches_solo",
        bool(iso.get("recovery_matches_solo")),
        "",
        checks,
        failures,
    )
    check(
        "A8_duplicate_cleanup_safe",
        True,
        "script completed past duplicate cleanup",
        checks,
        failures,
    )
    ...
    # A9 session hygiene
    check(
        "A9_sessions_zero_after_finalize",
        iso.get("sessions_after_finalize") == 0,
        str(iso.get("sessions_after_finalize")),
        checks,
        failures,
    )
